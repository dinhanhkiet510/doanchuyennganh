...
function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      let errMsg = err.response?.data?.error || err.response?.data?.message || "Registration failed!";
    ...
    >
      <div className="card shadow-lg p-4" style={{ width: "500px", borderRadius: "15px" }}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <button type="submit" className="btn w-100 text-white mb-3"
            style={{ background: "linear-gradient(to right, #ff512f, #dd2476)", border: "none", borderRadius: "8px" }}>
            Register
          </button>
        </form>
        ...
        <div className="d-flex justify-content-center mb-3">
          <button onClick={() => handleOAuthRegister("google")} className="btn btn-light d-flex align-items-center me-2" style={{ borderRadius: "8px", width: "48%" }}>
            <FaGoogle style={{ marginRight: "8px", color: "#DB4437" }} /> Google
          </button>
          <button onClick={() => handleOAuthRegister("facebook")} className="btn btn-light d-flex align-items-center" style={{ borderRadius: "8px", width: "48%" }}>
        ...
        <div className="mt-3 text-center">
          ...
          <button onClick={() => navigate("/login")} className="btn btn-link p-0" style={{ color: "#dd2476", fontWeight: "bold" }}>
            Login
          </button>
        </div>
      </div>
